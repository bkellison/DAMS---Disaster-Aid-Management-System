...
BEGIN
	...
	-- 3. Update Request
        ...
        SET status = 'pending'
		WHERE `match`.pledge = param_pledge_id AND (`match`.shipping_status not in ('shipped', 'delivered') OR (`match`.match_status != 'locked match'));
    
    -- 4. Remove unlocked matches
		UPDATE dr_events.`match`
        SET canceled_flag = 1 and match_status = 'canceled match'
		WHERE `match`.pledge_id = param_pledge_id AND (`match`.shipping_status not in ('shipped', 'delivered') OR (`match`.match_status != 'locked match'));
END ;;
DELIMITER ;
...
BEGIN
	...
	-- 1. Create Match
		INSERT INTO dr_events.`match` (pledge_id, request_id, match_status, match_quantity, match_type_id)
        VALUES (param_pledge_id, param_request_id, param_match_status, param_match_quantity, param_match_type_id);
...
BEGIN
    ...
    IF var_user_role = 'Donor' THEN
        ...
        -- JOIN dr_events.item ON request.
    ...
    ELSEIF var_user_role = 'Recipient' THEN
        ...
        JOIN dr_events.disaster_event ON request.event_id = disaster_event.event_id
        JOIN dr_events.category ON request.category_id = category.category_id
        -- JOIN dr_events. // join on items in pledge
...
BEGIN
    ...
    IF var_user_role = 'Recipient' THEN
		SELECT 
            ...
            ,CASE WHEN request.quantity - current_matches.total_matched < 0 THEN 0
				ELSE IFNULL(request.quantity - current_matches.total_matched, 0) END AS request_quantity_remaining
...
-- Dump completed on 2025-05-01  1:32:35
