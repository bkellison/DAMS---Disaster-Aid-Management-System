<template>
  <div class="manage-container">
    <div class="content-box">
      ...
      <p class="description">
        {{ isAdminObserver ? 'View donation items and available quantities for matching.' : 'Add or edit donation items and view available quantities for matching.' }}
      </p>

      <!-- Show warning for Admin Observer -->
      <div v-if="isAdminObserver" class="observer-warning">
        <strong>Admin Observer Mode:</strong> You can view items but cannot add, edit, or delete them.
      </div>
      ...
      <!-- Add/Edit Items Tab - Only show for non-observers -->
      <div v-if="activeTab === 'add' && !isAdminObserver" class="tab-content">
      ...
      <div v-if="activeTab === 'view'" class="tab-content">
        ...
        <div class="action-buttons">
          ...
          <AppButton v-if="!isAdminObserver" @click="goToMatchPage" variant="primary">Match Items to Requests</AppButton>
        </div>
      </div>
    </div>
  </div>
...
const activeTab = ref('view'); // Default to view tab
...
onMounted(() => {
  if (isAdminObserver.value) {
    activeTab.value = 'view'; // Force observers to view tab
  } else {
    activeTab.value = 'add'; // Default to add/edit for full admins
  }
});
...
const newItem = ref({
  ...
  description: '',
  quantity: 1,
  created_by: authStore.userId || 4  // Use current user ID
})
...
const fetchItems = async () => {
  if (isAdminObserver.value) return; // Don't fetch for observers since they can't edit
...
const addOrUpdateItem = async () => {
  // Prevent Admin Observer from modifying items
  if (isAdminObserver.value) {
    alert('Admin Observers cannot modify items.');
    return;
  }
...
// Cancel editing
const cancelEdit = () => {
  if (isAdminObserver.value) return; // Observers shouldn't be editing anyway
...
const resetForm = () => {
  newItem.value = {
    ...
    category_id: '',
    quantity: 1,
    created_by: authStore.userId || 4
  }
}

// Delete an item
const deleteItem = async (id) => {
  // Prevent Admin Observer from deleting items
  if (isAdminObserver.value) {
...
const startEditItem = (item) => {
  // Prevent Admin Observer from editing items
  if (isAdminObserver.value) {
...
onMounted(() => {
  if (!isAdminObserver.value) {
    fetchItems() // Only fetch items if user can edit them
  }
  fetchCategories()
  fetchItemAvailability()
})
